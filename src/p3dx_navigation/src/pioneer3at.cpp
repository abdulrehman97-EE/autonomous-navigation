 ...
 * rosrun gmapping slam_gmapping scan:=/SANCO_4750/Sick_LMS_291/laser_scan/layer0 _xmax:=30 _xmin:=-30 _ymax:=30 _ymin:=-30
 * _delta:=0.2
 */
...
void updateSpeed() {
  ...
  for (int i = 0; i < NMOTORS; ++i) {
    ...
    set_velocity_client = n->serviceClient<webots_ros::set_float>(std::string("SANCO_4750/") + std::string(motorNames[i]) + std::string("/set_velocity"));
...
void broadcastTransform() {
  ...
  br.sendTransform(tf::StampedTransform(transform, ros::Time::now(), "base_link", "SANCO_4750/Sick_LMS_291"));
}
...
void quit(int sig) {
  ROS_INFO("User stopped the 'SANCO_4750' node.");
...
int main(int argc, char **argv) {
  ...
  // create a node named 'SANCO_4750' on ROS network
  ros::init(argc, argv, "SANCO_4750", ros::init_options::AnonymousName);
  ...
  timeStepClient = n->serviceClient<webots_ros::set_int>("SANCO_4750/robot/time_step");
  ...
  for (int i = 0; i < NMOTORS; ++i) {
    ...
    set_position_client = n->serviceClient<webots_ros::set_float>(std::string("SANCO_4750/") + std::string(motorNames[i]) +
    ...
    ros::ServiceClient set_velocity_client;
    webots_ros::set_float set_velocity_srv;
    set_velocity_client = n->serviceClient<webots_ros::set_float>(std::string("SANCO_4750/") + std::string(motorNames[i]) +
  ...
  set_lidar_client = n->serviceClient<webots_ros::set_int>("SANCO_4750/Sick_LMS_291/enable");
  lidar_srv.request.value = TIME_STEP;
  if (set_lidar_client.call(lidar_srv) && lidar_srv.response.success) {
    ROS_INFO("Lidar enabled.");
    sub_lidar_scan = n->subscribe("SANCO_4750/Sick_LMS_291/laser_scan/layer0", 10, lidarCallback);
  ...
  set_GPS_client = n->serviceClient<webots_ros::set_int>("SANCO_4750/gps/enable");
  GPS_srv.request.value = 32;
  if (set_GPS_client.call(GPS_srv) && GPS_srv.response.success) {
    sub_GPS = n->subscribe("SANCO_4750/gps/values", 1, GPSCallback);
  ...
  set_inertial_unit_client = n->serviceClient<webots_ros::set_int>("SANCO_4750/inertial_unit/enable");
  inertial_unit_srv.request.value = 32;
  if (set_inertial_unit_client.call(inertial_unit_srv) && inertial_unit_srv.response.success) {
    sub_inertial_unit = n->subscribe("SANCO_4750/inertial_unit/quaternion", 1, inertialUnitCallback);
  ...
  set_accelerometer_client = n->serviceClient<webots_ros::set_int>("SANCO_4750/accelerometer/enable");
  ...
  set_camera_client = n->serviceClient<webots_ros::set_int>("SANCO_4750/camera/enable");
  ...
  set_gyro_client = n->serviceClient<webots_ros::set_int>("SANCO_4750/gyro/enable");
  ...
  ROS_INFO("You can now start the creation of the map using 'rosrun gmapping slam_gmapping "
           "scan:=/SANCO_4750/Sick_LMS_291/laser_scan/layer0 _xmax:=30 _xmin:=-30 _ymax:=30 _ymin:=-30 _delta:=0.2'.");
